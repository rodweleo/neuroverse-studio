...
const AgentDiscovery = () => {
  ...
  const filteredAndSortedAgents = useMemo(() => {
    let filtered = publicAgents?.filter((agent) => {
      const matchesSearch =
        !searchQuery ||
    ...
    return filtered;
  }, [searchQuery, selectedTags, sortBy, publicAgents]);
        ...
        <div className="glassmorphic p-6 rounded-lg border border-neon-blue/20">
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between mt-6 pt-6 border-t border-neon-blue/20">
            ...
            <p className="text-muted-foreground text-sm">
              Found{" "}
              {filteredAndSortedAgents ? filteredAndSortedAgents.length : 0}{" "}
              agent{filteredAndSortedAgents?.length > 1 ? "s" : ""}
            </p>
          </div>
        </div>
